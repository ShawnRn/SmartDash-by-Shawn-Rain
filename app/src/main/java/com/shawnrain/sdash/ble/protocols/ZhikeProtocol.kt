...
class ZhikeProtocol : ControllerProtocol {
    ...
    // All-zero frame burst detection: track consecutive all-zero frames
    private var consecutiveAllZeroFrames = 0
    private var lastValidRealtimeMetrics: VehicleMetrics? = null
    ...
    private fun processFrame(
        ...
        when (cmd) {
            CMD_REALTIME -> {
                ...
                // --- All-zero frame detection: suppress invalid realtime bursts ---
                val isAllZeroFrame = voltage < 2f && abs(busCurrent) < 0.1f &&
                        abs(rpmSigned) < 1f && sanitizedSpeed < 0.1f &&
                        normalizedPhaseCurrent < 0.1f

                if (isAllZeroFrame) {
                    consecutiveAllZeroFrames++
                    if (consecutiveAllZeroFrames == 10) {
                        AppLogger.w(TAG, "检测到连续 10 帧全 0 实时数据，抑制无效遥测流")
                    }
                    // Don't emit all-zero frames; keep last valid metrics alive
                    return
                } else {
                    if (consecutiveAllZeroFrames >= 10) {
                        AppLogger.i(TAG, "遥测流恢复正常 (之前连续 ${consecutiveAllZeroFrames} 帧全 0)")
                    }
                    consecutiveAllZeroFrames = 0
                }
