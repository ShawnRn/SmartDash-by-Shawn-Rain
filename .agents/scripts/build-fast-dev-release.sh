...
if should_route_remote_build; then
  ...
  if [[ -z "$REMOTE_FAST_DEV_RELEASE_APK_PATH" || -z "$REMOTE_ARCHIVE_APK" || -z "$REMOTE_BUILD_LOG" ]]; then
  ...
  copy_remote_file_to_local "$REMOTE_TARGET" "$REMOTE_FAST_DEV_RELEASE_APK_PATH" "$FAST_DEV_RELEASE_APK_PATH"
