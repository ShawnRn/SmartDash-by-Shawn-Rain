...
if "${ADB_PREFIX[@]}" shell pm list packages | grep -q "package:com.vivo.bsptest"; then
  echo "[智能路由] 检测到真机已安装 com.vivo.bsptest，将安装 vivo 渠道的 devRelease APK..."
  INSTALL_APK_PATH="$(extract_output_var "$BUILD_OUTPUT_FILE" "DEV_VIVO_APK")"
else
  echo "[智能路由] 未在真机检测到 com.vivo.bsptest，将安装普通渠道的 devRelease APK..."
  INSTALL_APK_PATH="$(extract_output_var "$BUILD_OUTPUT_FILE" "DEV_NORMAL_APK")"
fi

if [[ -z "$INSTALL_APK_PATH" ]]; then
  echo "Build output did not include targeted APK path." >&2
  exit 1
fi
